...
public class Step01VariableTest extends PlainTestCase {
    ...
    public void test_variable_writing() {
        ...
        // 初期化せずに変数にアクセスしたいユースケースが色々ありそうなインスタンス変数・グローバル変数では、初期化しなくてもコンパイルエラーにならないようにデフォルト値を入れるが、ローカル変数は初期化せずにアクセスしたいユースケースがないような気がするので宣言時に初期値を入れないとエラーになるのか？（参考：https://teratail.com/questions/52814）
        // TODO jflute インスタンス変数がクラスの直下でメソッドの外に定義する変数だとすると、writing method の下でインスタンス変数の piari を定義することはできない気がしました。この理解が正しいか、教えていただきたいです by m.inoue (2024/07/05)
        // TODO jflute ローカル変数では初期化しないとコンパイルエラーになるが、インスタンス変数やグローバル変数では初期化しなくてもコンパイルエラーにならない背景について教えていただきたいです。私としては、2行上の理解 (「ローカル変数では...」) でいます by m.inoue (2024/07/05)
    }
