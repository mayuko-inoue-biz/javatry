...
//        Ticket ticket = new Ticket(TicketType.ONE_DAY_PASSPORT); // also here
    ...
    // 
    // done jflute 1on1にて、もういっこの再利用に関する思考のエクササイズをする予定 (2024/11/01)
    // [1on1でのふぉろー] WindowsのFileSeparatorという概念で意味を一致させることができる。
    // ほぼ変わらないものなので、修正時に一箇所直せば良いメリットはあんまり薄いけど、
    // 可読性のために名前付けして一元管理するというメリットはある。
    // (再利用のきっかけ、大きく二つ↑)
    //
    // [1on1でのふぉろー] WindowsとOldWindowsのクラス階層についてのよもやま話もした。
    // 本気で階層を綺麗にしていくのであれば、色々な形がある。
    //
    // [1on1でのふぉろー] 中途半端な構造でオブジェクト指向すると、そりゃわかりにくくなる。
    // やるからにはしっかり概念を整理整頓した上でコードを積み上げていかないといけない。
    // (勉強しないでさっくりなんとなくで良い構造ができるっていうものではない)
    ...
    public void test_objectOriented_writing_withPackageRefactoring() {
        ...
        // インスタンスの方が広くなるので、他にもpublicメソッドある場合はちょっと抵抗ある。
        // けど、この場合は1publicメソッドクラスと言ってもいいようがものなので、全然問題ない。
    }
